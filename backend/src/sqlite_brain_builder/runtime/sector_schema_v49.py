...
def ensure_sector_schema(db: Path, lane_key: str):
    ...
    con.execute(
        ...
        (
            ...
            1,
            "Schema exists so model does not invent schema later.",
            now(),
        ),
    )
    con.commit()
...
def ensure_all_sector_schemas(workspace_dir: str, brain_name: str):
        ...
        json.dumps(
            {
                ...
                "code_mmd_policy": "routes/pages -> route file -> imports/services -> deps/tools -> artifacts",
                "created_at": now(),
            },
            indent=2,
        ),
    ...
    return {"brain_root": str(brain_root), "sector_count": len(index)}
