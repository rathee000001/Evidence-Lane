...
from sqlite_brain_builder.runtime.sector_schema_v48 import cleanup_non_code_mmds
...
def import_targets(con, from_path, limit=5):
    return rows(
        con,
        """
...
def write_route_first_code_mmd(db: Path, out: Path):
    ...
    for idx, (route_path, route_type, file_id, file_path) in enumerate(routes[:80]):
        ...
        lines.append(f'    {import_node}["{safe(import_label, 140)}"]:::service')
    ...
    lines += [
        "",
        "  subgraph DEPENDENCIES[tools and dependencies]",
        "    direction TB",
    ]
    ...
    lines += [
        "",
        "  subgraph ARTIFACTS[artifact payload groups]",
        "    direction TB",
    ]
