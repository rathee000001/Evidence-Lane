...
def _render_one(mmd: Path, cb=None, tracker=None):
    svg = mmd.with_suffix(".svg")
    png = mmd.with_suffix(".png")
    png4k = mmd.with_name(mmd.stem + "_4K.png")
    crystal = mmd.with_name(mmd.stem + "_CRYSTAL.png")

    cmd_svg = [_mmdc(), "-i", str(mmd), "-o", str(svg), "-b", "white"]
    run = subprocess.run(cmd_svg, capture_output=True, text=True, timeout=1200)
    if run.returncode != 0:
        raise RuntimeError(f"SVG_RENDER_FAILED for {mmd}: {run.stderr[:2500]}")

    cmd_4k = [_mmdc(), "-i", str(mmd), "-o", str(png4k), "-b", "white", "-w", "7680", "-H", "4320"]
    run = subprocess.run(cmd_4k, capture_output=True, text=True, timeout=1200)
    if run.returncode != 0:
        raise RuntimeError(f"4K_RENDER_FAILED for {mmd}: {run.stderr[:2500]}")

    shutil.copy2(png4k, png)

    cmd_crystal = [_mmdc(), "-i", str(mmd), "-o", str(crystal), "-b", "white", "-w", "15360", "-H", "8640"]
    run = subprocess.run(cmd_crystal, capture_output=True, text=True, timeout=1800)
    if run.returncode != 0:
...
def render_topology_only(workspace_dir: str, brain_name: str, progress=None):
    from sqlite_brain_builder.runtime.topology_writer_v48 import regenerate_topology_from_existing_brain
